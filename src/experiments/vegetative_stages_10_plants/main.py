...
def augment_axes_inital_state(df, nplants):
    dfo = df.copy()
    for i in range(1,nplants):
        dfn = df.copy()
        dfn["plant"] = i+1
        dfn.at[0, "mstruct"] = max(0.01, dfn.loc[0, "mstruct"] + 0.01*np.random.randn(1))
        dfn.at[0, "minerals_plant"] = max(0.001, dfn.loc[0, "minerals_plant"] + 0.001 * np.random.randn(1))
        dfn.reindex(np.arange(len(df)*i, len(df)*(i+1), dtype=np.int))

        dfo = pd.concat((dfo, dfn))
    return dfo


def update_variable(df, idx, name):
    df.at[idx, name] = max(0.1*df.loc[idx, name], df.loc[idx, name] * (1 + 0.1*np.random.randn(1)))


def augment_elements_inital_state(df, nplants):
    dfo = df.copy()
    for i in range(1,nplants):
        dfn = df.copy()
        dfn["plant"] = i+1
        for j in range(len(dfn)):
            for var in ["length", "Nstruct", "proteins", "sucrose", "green_area", "cytokinins", "max_proteins",
                        "max_mstruct"]:
                update_variable(dfn, j, var)

        update_variable(dfn, 3, "fructan")
        for j in [0, 2, 5]:
            update_variable(dfn, j, "Wmax")
...
def augment_hiddenzones_inital_state(df, nplants):
    dfo = df.copy()
    for i in range(1, nplants):
        dfn = df.copy()
        dfn["plant"] = i+1
        for j in range(len(dfn)):
            for var in ["hiddenzone_age", "leaf_L", "sucrose", "amino_acids", "fructan", "proteins", "leaf_enclosed_mstruct",
                        "internode_enclosed_mstruct", "mstruct", "leaf_enclosed_Nstruct", "internode_enclosed_Nstruct",
                        "Nstruct"]:
                update_variable(dfn, j, var)
        dfn.reindex(np.arange(len(df)*i, len(df)*(i+1), dtype=np.int))
...
def augment_organs_inital_state(df, nplants):
    dfo = df.copy()
    for i in range(1,nplants):
        dfn = df.copy()
        dfn["plant"] = i+1
        for j in range(len(dfn)):
            for var in ["sucrose", "amino_acids"]:
                update_variable(dfn, j, var)
        dfn.reindex(np.arange(len(df)*i, len(df)*(i+1), dtype=np.int))
...
def augment_soils_inital_state(df, nplants):
    dfo = df.copy()
    for i in range(1,nplants):
        dfn = df.copy()
        dfn["plant"] = i+1
        for j in range(len(dfn)):
            for var in ["nitrates", "Tsoil"]:
                update_variable(dfn, j, var)
        dfn.reindex(np.arange(len(df)*i, len(df)*(i+1), dtype=np.int))
...
def augment_adel_data(df, nplants):
    dfo = df.copy()
    for i in range(1, nplants):
        dfn = df.copy()
        dfn["plant"] = i + 1
        for j in range(len(df)):
            for var in ["incT", "emf1", "azT"]:
                update_variable(dfn, j, var)
        dfn.reindex(np.arange(len(df) * i, len(df) * (i + 1), dtype=np.int))
...
def main(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
         ...
         N_fertilizations=None, PLANT_DENSITY=None, update_parameters_all_models=None, INTER_ROW=0.15,
    ...
    if PLANT_DENSITY is None:
        PLANT_DENSITY = {1: 250.}
    if n_plants > 1:
        PLANT_DENSITY = {i+1: 250. / n_plants for i in range(n_plants)}
    ...
    if run_from_outputs:
        ...
        for inputs_filename in (AXES_INITIAL_STATE_FILENAME,
            ...
            if inputs_filename == AXES_INITIAL_STATE_FILENAME:
                inputs_dataframe = augment_axes_inital_state(inputs_dataframe, n_plants)
            elif inputs_filename == ORGANS_INITIAL_STATE_FILENAME:
                inputs_dataframe = augment_organs_inital_state(inputs_dataframe, n_plants)
            elif inputs_filename == HIDDENZONES_INITIAL_STATE_FILENAME:
                inputs_dataframe = augment_hiddenzones_inital_state(inputs_dataframe, n_plants)
            elif inputs_filename == ELEMENTS_INITIAL_STATE_FILENAME:
                inputs_dataframe = augment_elements_inital_state(inputs_dataframe, n_plants)
            elif inputs_filename == SOILS_INITIAL_STATE_FILENAME:
                inputs_dataframe = augment_soils_inital_state(inputs_dataframe, n_plants)
    ...
    stand = AgronomicStand(sowing_density=PLANT_DENSITY[1],
    ...
    adel_wheat = AdelDyn(seed=1,
                         scene_unit='m',
                         leaves=echap_leaves(xy_model='Soissons_byleafclass', top_leaves=0),
                         stand=stand,
                         nplants=n_plants)
    axeT_df = pd.read_csv(os.path.join(INPUTS_DIRPATH, 'axeTable.csv'))
    axeT_df = augment_adel_data(axeT_df, n_plants)
    ...
    if run_simu:
        ...
        try:
            ...
            for t_caribu in range(START_TIME, SIMULATION_LENGTH, SENESCWHEAT_TIMESTEP):
                print(f"[{time.time() - start_time:.2f}] New epoch.")
    ...
    if generate_graphs:
        ...
        bchmk.insert(2, "lamina_W_Lg",  bchmk["lamina_Wmax"] / bchmk["lamina_Lmax"], True)
...
if __name__ == '__main__':
    main(2000, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=False, run_from_outputs=False,
         show_3Dplant=False,
